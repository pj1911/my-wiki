...
\section*{A tiny grounding example}
...
\section*{Baseline: always predict the average}
...
\section*{Simple (one-feature) linear regression}
...
\paragraph{Optimal Parameters $w_0^*$ and $ w_1^*$:}
...
\paragraph{Metrics for goodness of fit}
...
\paragraph{How do we know if the fit is good?}
\begin{itemize}
  \item \textbf{Residual variance} $\boldsymbol{\sigma_e^2}$ (or RMSE $=\sqrt{\sigma_e^2}$) should be \emph{small relative to the scale of $y$}. 
        Rule of thumb: $\mathrm{RMSE} \ll \text{SD}(y)=\sigma_y$ is good.
  \item \textbf{$\boldsymbol{R^2}$} close to $1$ means the model explains most variance.
  \item \textbf{Beating the baseline}: ensure $\sigma_e^2 < \sigma_y^2$ (equivalently $R^2>0$). If not, the mean-only model is better.
  \item \textbf{Out-of-sample}: check test/validation $R^2$ or RMSE. A good fit \emph{generalizes} (train and test metrics are similar).
  \item \textbf{Residual diagnostics}: residuals should look like noise (no trend vs.\ $\hat y$ or $x$, roughly constant spread, few large outliers).
  \item \textbf{Adjusted $R^2$ (for multiple $x$)}: prefer higher adjusted $R^2$; it penalizes unnecessary features.
\end{itemize}

\section*{Multiple linear regression (many features)}
...
\section*{Linear basis function regression (same idea with richer inputs)}
...
\section*{Why the normal equations appear (gradient view)}
...
\section*{Quick OLS recipe (practical path)}

\begin{enumerate}
  \item Gather data $(\mathbf{x}_i,y_i)_{i=1}^n$ \emph{including an intercept column of ones}.
  \item Choose your features/bases $\boldsymbol{\phi}(\cdot)$ (raw, polynomial, binned, etc.).
  \item Fit by OLS: $\mathbf{w}^*=(\mathbf{B}^\top\mathbf{B})^{-1}\mathbf{B}^\top\mathbf{y}$ or a numeric solver (QR/SVD are more stable).
  \item Predict: $\hat y=\langle\boldsymbol{\phi}(\mathbf{x}),\mathbf{w}^*\rangle$.
  \item Evaluate on held-out data (MSE, $R^2$).
\end{enumerate}
